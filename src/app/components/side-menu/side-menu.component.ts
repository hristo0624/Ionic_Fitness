...
export class SideMenuComponent implements OnInit {
  ...
  public get isPaidUser(): boolean {
    const user = this.getUser();
    return user ? user.is_paid_user : false;
  }

  public get hasActiveAdvisor(): boolean {
    // TODO: Add active advisor check.
    // It should be stupid easy to figure out if the user has an active Advisor
    // without having to reach out to the API (which is not cached!) and then
    // figure that out. For now, let's just assume this is true.
    return true;
  }
